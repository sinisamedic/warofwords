...
func setup(sheet: Texture2D, opponent: bool) -> void:
	setup_region(sheet,Rect2(1140,70,250,250) if opponent else Rect2(290,90,250,250),true)

func setup_region(sheet: Texture2D, region: Rect2, chroma: bool) -> void:
	texture = sheet
	region_enabled = true
	region_rect = region
	scale = Vector2.ONE * 48.0/region.size.x
...
void fragment() {
 ...
 float key = c.a * (chroma ? 1.0-smoothstep(.35,.7,min(c.r,c.b)-c.g) : 1.0);
 COLOR = vec4(mix(vec3(.06,.15,.23),c.rgb,key),mask);
}"""
	material = ShaderMaterial.new()
	material.shader = shader
	material.set_shader_parameter("chroma",chroma)
