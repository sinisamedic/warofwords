...
func run_tests() -> void:
	...
	check(scene.enemy_actor.rotation==0 and scene.enemy_actor.material.get_shader_parameter("dissolve")>.1 and scene.hero.modulate.a==1,"defeated enemy breaks apart without rotating the whole sprite")
	scene.finish(true)
	...
	check(scene.enemy_actor.material.get_shader_parameter("dissolve")==0.0,"next duel clears enemy disintegration")
	...
	scene.result_delay=scene.DEFEAT_DURATION*.5; scene.update_actors()
	check(scene.ended and not scene.won and scene.hero.rotation==0 and scene.hero.joints.Body.position.y> -45 and scene.enemy_actor.modulate.a==1,"hero collapses at joints while the root and opponent stay upright")
	check(scene.hero.joints.FrontLeg.surface.polygon!=scene.hero.joints.FrontLeg.rest,"hero knees deform during collapse")
	check(absf(scene.hero.joints.Head.rotation-scene.hero.joints.FrontArm.rotation)>.1,"head and weapon arm follow distinct death motions")
	var death_body: Transform2D=scene.hero.joints.Body.transform
	scene.update_actors()
	check(scene.hero.joints.Body.transform==death_body,"death pose is deterministic at the same timeline position")
	scene.result_delay=scene.DEFEAT_DURATION*.18; scene.update_actors()
	check(scene.hero.modulate.a==1 and scene.hero.joints.Body.position.y> -15,"hero visibly reaches the ground before fading")
	...
	check(not scene.hero.defeated and scene.hero.joints.Body.rotation==0 and scene.hero.joints.FrontLeg.surface.polygon==scene.hero.joints.FrontLeg.rest,"new duel restores torso and leg mesh after hero death")
