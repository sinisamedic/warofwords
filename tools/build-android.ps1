...
& $Godot --headless --path $project --script res://tests/test_tactics.gd
...
& $Godot --headless --path $project --script res://tests/test_profile_deletion.gd
if ($LASTEXITCODE -ne 0) { throw 'Profile deletion tests failed.' }
...
$apk = Join-Path $outputDir 'WarOfWords-0.1.25-android.apk'
if ($UnsignedCheck) {
    # Validate export without signing the artifact with this machine's new debug key.
    $apk = Join-Path $repo '.local/WarOfWords-0.1.25-UNSIGNED-CHECK.apk'
