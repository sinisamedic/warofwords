...
let attempt={id:'12345678-1234-1234-1234-123456789abc',user_id:'owner',language:'en',adjacent:true,version:'daily-v1',seed:123456,started_at:new Date(Date.now()-125000).toISOString(),finished_at:null};
let patches=0;
globalThis.fetch=async(url,options={})=>{
  ...
  if(url.endsWith('/rpc/daily_leaderboard')) return Response.json({rows:[],total:0});
  if(url.endsWith('/rpc/daily_start')) return Response.json({...attempt,day:'2026-09-17'});
  throw Error('Unexpected request');
};
await import('../supabase/functions/daily/index.ts');
const call=(body,token='valid')=>handler(new Request('https://test.invalid/functions/v1/daily',{method:'POST',headers:{Authorization:`Bearer ${token}`},body:JSON.stringify({version:'daily-v1',language:'en',adjacent:true,...body})}));
...
assert.equal((await call({action:'leaderboard',version:'daily-v2'})).status,400);
