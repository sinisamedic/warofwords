...
func _save_scene(_params: Dictionary) -> Dictionary:
	...
	# Godot 4.7 disallows the save progress dialog during deferred calls.
	await get_tree().process_frame
	var save_error := editor_plugin.get_editor_interface().save_scene()
	if save_error != OK:
		return _err("Failed to save scene: error %d" % save_error)
